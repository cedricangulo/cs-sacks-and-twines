...
<dialog id="add-inventory-dialog" class="w-full dialog sm:max-w-5xl max-h-164" aria-labelledby="add-inventory-dialog-title" aria-describedby="add-inventory-dialog-description" data-inventory-dialog onclick="if (event.target === this) this.close()">
  <div class="max-w-5xl">
    ...
    <section class="overflow-y-auto scrollbar">
      <form id="inventory-form" class="grid gap-4 pb-4 form" data-inventory-form action="<?= htmlspecialchars(routeUrl('/inventory/save'), ENT_QUOTES, 'UTF-8') ?>" method="POST">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-4">
            <div class="relative grid gap-3" data-existing-item-panel>
              <label class="label" for="inventory-item-search">Item Name</label>
            ...
            <div class="grid gap-3" hidden data-new-item-panel>
              <label class="label" for="inventory-item-name">Item Name</label>
              <input
                class="w-full input"
                type="text"
          ...
          <div class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div class="grid gap-3" data-field-group="category">
                <div class="flex items-center justify-between gap-4">
                ...
                <select class="w-full select" id="inventory-item-category" name="category" disabled data-field-input="category">
              ...
              <div class="grid gap-3" data-field-group="unit">
                <div class="flex items-center justify-between gap-4">
                  <label class="label" for="inventory-item-unit">Unit / Measurement</label>
                  <button type="button" class="hidden text-xs font-medium text-primary" data-edit-field="unit">Edit</button>
                </div>
                <select class="w-full select" id="inventory-item-unit" name="base_uom" disabled required data-field-input="unit">
            ...
            <div class="grid gap-3" data-field-group="supplier">
              <div class="flex items-center justify-between gap-4">
                <label class="label" for="inventory-item-supplier">Supplier</label>
                <button type="button" class="hidden text-xs font-medium text-primary" data-edit-field="supplier">Edit</button>
              </div>
              <select class="w-full select" id="inventory-item-supplier" name="supplier_id" disabled required data-field-input="supplier">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="grid gap-3">
                ...
                <input class="w-full input" type="text" id="inventory-item-sku" name="sku_code" value="Generated on save" readonly data-field-input="sku" />
              </div>

              <div class="grid gap-3">
                <label class="label" for="inventory-item-batch">Batch ID</label>
                <input class="w-full input" type="text" id="inventory-item-batch" name="batch_code" value="Generated on save" readonly data-field-input="batch" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="grid gap-3">
                ...
                <input class="w-full input" type="number" id="inventory-item-quantity" name="quantity_received" min="0.01" step="0.01" placeholder="0.00" required data-field-input="quantity" />
              </div>

              <div class="grid gap-3">
                <label class="label" for="inventory-item-cost">Total Procurement Cost</label>
                <input class="w-full input" type="number" id="inventory-item-cost" name="unit_cost" min="0.01" step="0.01" placeholder="0.00" required data-field-input="cost" />
              </div>
            </div>
          </div>
      </form>
